...
if __name__ == '__main__':
    fnt = Font("PixelCrypt.ttf", 16)
    face = freetype.Face("PixelCrypt.ttf")
    dic = {}
    for c in range(33,126+1):
        print(chr(c))
        face.load_char(c, freetype. | freetype.FT_LOAD_TARGET_MONO)
        states = [bool(b) for b in ch.pixels]
        dic[chr(c)] = states
    for entry in dic:
        print(entry)
        for i in range(len(dic[entry])):
            if (dic[entry][i]):
                print 'O',
            else:
                print ' ',
